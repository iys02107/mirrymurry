...
import com.codepresso3.mirrymurry.dto.BookHistDto;
...
public class BookController {
    ...
    @GetMapping("/bookHist")
    public String bookHist(Model model, Principal principal){
        Member member = memberMapper.findByEmail(principal.getName());
        List<BookHistDto> bookHistDtoList = bookService.getBookHistList(member.getMember_id());
        model.addAttribute("bookHistDtoList", bookHistDtoList);
        model.addAttribute("name", member.getUser_name());
        return "member/bookHist";
    }
