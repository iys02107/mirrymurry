...
public class StoreImgService {
    ...
    public void updateStoreImg(Integer storeImgId, MultipartFile storeImgFile) throws Exception {

        if (!storeImgFile.isEmpty()) {
            StoreImg savedStoreImg = storeImgMapper.findById(storeImgId);
            String oriImgName = storeImgFile.getOriginalFilename();
            ...
            savedStoreImg.updateItemImg(storeImgId, imgName, oriImgName, imgUrl);

            storeImgMapper.updateImg(savedStoreImg);
        }
    }
